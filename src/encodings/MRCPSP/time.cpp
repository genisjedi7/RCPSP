...
Time::Time(MRCPSP * instance, AMOPBEncoding amopbenc) : MRCPSPEncoding(instance)  {
	this->maxsat = maxsat;
	this->amopbenc = amopbenc;
}

SMTFormula * Time::encode(int lb, int ub){
...
void Time::setModel(const EncodedFormula & ef, int lb, int ub, const vector<bool> & bmodel, const vector<int> & imodel){
	int N = ins->getNActivities();
...
bool Time::narrowBounds(const EncodedFormula & ef, int lastLB, int lastUB, int lb, int ub){
	int N = ins->getNActivities();
